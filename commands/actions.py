...
def explain_clipboard_code(session_id: str):
    """Sends the clipboard's content to the LLM for an explanation."""
    try:
        code = pyperclip.paste()
        if not code: return "Your clipboard is empty."
        
        prompt = f"Please explain what this code does in simple terms:\n\n---\n{code}\n---"
        
        # Pass the session_id to the LLM call
        explanation = get_gpt_response(prompt, session_id, mode="developer")
        return explanation
    except Exception as e:
        return f"Sorry, I couldn't read your clipboard: {e}"
...
def comment_on_issue(repo_name: str, issue_num: int, comment: str):
    if not GITHUB_PAT: return "Error: GITHUB_PAT not found."
    try:
        repo = g.get_repo(repo_name)
        issue = repo.get_issue(number=issue_num)
        issue.create_comment(comment)
        return f"Done. I've posted your comment to issue number {issue_num}."
    except Exception as e:
        return f"Could not post comment: {e}"
...
def set_working_directory(path_query: str) -> tuple[str, str]:
    search_paths = [os.path.expanduser("~/Desktop"), os.path.expanduser("~/Documents")]
    for path in search_paths:
        for root, dirs, files in os.walk(path):
            if path_query.lower() in [d.lower() for d in dirs]:
                for dir_name in dirs:
                    if path_query.lower() == dir_name.lower():
                        project_path = os.path.join(root, dir_name)
                        return f"Okay, working directory set to {dir_name}.", project_path
    return f"Sorry, I couldn't find a folder named '{path_query}'.", None
